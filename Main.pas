unit Main; //модуль главного меню
...
procedure TMainForm.ButtonDeviceClick(Sender: TObject); // кнопка для отображения раздела "Устройство велосипеда"
begin
Device.ShowModal;
end;

procedure TMainForm.FormActivate(Sender: TObject); // отображение логина авторизованного пользователя
begin
WelcomeLabel.Caption :='Вы авторизовались как ' + Reg.VeloLandUsers.FieldByName('UserLogin').AsString + '!';
end;

procedure TMainForm.FormClose(Sender: TObject; var Action: TCloseAction); // заркыть полностью приложении при закрытии главного меню
begin
...
procedure TMainForm.FormCreate(Sender: TObject); // загрузка картинок, музыки, установка расположения элементов управления
begin
...
procedure TMainForm.FormShow(Sender: TObject); // отобразить счёт пользователя
begin
Score.Caption := 'Текущий счёт:' + #13#10 + Reg.VeloLandUsers.FieldByName('UserScore').AsString;
end;

procedure TMainForm.GameButtonClick(Sender: TObject); // кнопка для перехода на игру "Парные картинки с велосипедами"
begin
...
procedure TMainForm.MusicTimerTimer(Sender: TObject); // проверка закончилась ли музыка и отображение текущей композиции
begin
...
procedure TMainForm.AboutProgClick(Sender: TObject); // отображение окна "О программе"
begin
AProgramm.ShowModal;
end;

procedure TMainForm.ResetProgressClick(Sender: TObject); // сбросить прогресс пользователя
...
procedure TMainForm.ExitAccountClick(Sender: TObject); // выход из аккаунта
var buttonSelected: integer;
begin
...
procedure TMainForm.TestsMenuClick(Sender: TObject); // отображение раздела "Тесты"
begin
Test.Show;
MainForm.Hide;
end;

procedure TMainForm.DeviceMenuClick(Sender: TObject); // отображение раздела "Устройство велосипеда"
begin
Device.ShowModal;
end;

procedure TMainForm.TypesMenuClick(Sender: TObject); // отображение раздела "Типы велосипедов"
begin
Types.ShowModal;
end;

procedure TMainForm.TricksMenuClick(Sender: TObject); // отображение раздела "Трюки на велосипеде"
begin
Tricks.ShowModal;
end;

procedure TMainForm.RoutesMenuClick(Sender: TObject); // отображение раздела "Маршруты Гродно"
begin
Routes.ShowModal
end;

procedure TMainForm.PicturesGamesMenuClick(Sender: TObject); // игры "Парные картинки с велосипедами
begin
if Reg.VeloLandUsers.FieldByName('UserScore').AsInteger < 2700 then
  ShowMessage('Недостаточно очков для открытия игры! Наберите минимум 2700 очков!')
else
...
procedure TMainForm.HelpClick(Sender: TObject); // вызов справки
begin
ShellExecute(0, PChar('Open'),PChar('VeloLandHelp.chm'),nil,nil,SW_SHOW);
end;

procedure TMainForm.NextMusicClick(Sender: TObject); //переключение музыки вперёд
begin
 if music_index = MusicList.Count-1 then
            music_index := 0
else
...
procedure TMainForm.PrevMusicClick(Sender: TObject); // переключение музыки назад
begin
...
procedure TMainForm.RouteButtonClick(Sender: TObject); // кнопка для отображения раздела "Маршруты Гродно"
begin
Routes.ShowModal;
end;

procedure TMainForm.ButtonSelectionClick(Sender: TObject); // кнопка отображения раздела "Виды велосипедов"
begin
Types.ShowModal;
end;

procedure TMainForm.TestsButtonClick(Sender: TObject); // кнопка отображения раздела "Тесты"
begin
Test.Show;
MainForm.Hide;
end;

procedure TMainForm.ExitButtonClick(Sender: TObject); // кнопка выхода из приложения
var buttonSelected: integer;
begin
...
procedure TMainForm.TitleColorTimer(Sender: TObject); // переливание текста "Планета велосипедов"
begin
...
procedure TMainForm.VolumeClick(Sender: TObject); // кнопка включения и выключения музыки
begin
