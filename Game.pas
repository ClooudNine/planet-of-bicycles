unit Game; // модуль игры "Парные картинки"
...
procedure TPicturesGame.FormClose(Sender: TObject; var Action: TCloseAction); // начисление очков в БД при закрытии формы
begin
Timer.Enabled := False;
Time.Visible := False;
Field.Visible := False;
...
procedure TPicturesGame.FormCreate(Sender: TObject); // базовое расположение элементов и загрузка картинок
begin
...
procedure TPicturesGame.LevelOneButtonClick(Sender: TObject); //выбор первого уровня игры
begin
...
procedure TPicturesGame.LevelTwoButtonClick(Sender: TObject); //выбор второго уровня игры
begin
...
procedure TPicturesGame.HelpClick(Sender: TObject); // вызов справка
begin
ShellExecute(0, PChar('Open'),PChar('VeloLandHelp.chm'),nil,nil,SW_SHOW);
end;

procedure TPicturesGame.AboutProgrClick(Sender: TObject); // вызов окна "О программе"
begin
AProgramm.ShowModal;
end;

procedure TPicturesGame.LevelThreeButtonClick(Sender: TObject); // выбор третьего уровня игры
begin
if Reg.VeloLandUsers.FieldByName('UserGameScore').AsInteger < 600 then // проверка пройден ли предыдущий уровень
    begin
      ShowMessage('Для прхождения этого уровня Вы должны пройти предыдущий!');
      exit;
    end;
...
procedure TPicturesGame.LevelFourButtonClick(Sender: TObject); // выбор четвёртого уровня игры
begin
...
procedure TPicturesGame.BackToMainMenuClick(Sender: TObject); // кнопка возвращения на главное меню
begin
PicturesGame.Close;
MainForm.Show;
end;

procedure TPicturesGame.FieldMouseDown(Sender: TObject; Button: TMouseButton; //нажатие на клетку поля
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TPicturesGame.FieldPaint(Sender: TObject); // рисование поля
begin
ShowPole;
end;
...
begin
  ...
  begin
    ...
    Time.Visible := False;
    Reg.VeloLandUsers.Edit;
    if (SetLevel = 'One') and (Reg.VeloLandUsers.FieldByName('UserGameScore').AsInteger < 200) then // начисление очков в зависимости от того, проходился ли уровень раньше
    ...
    else if (SetLevel = 'Four') and (Reg.VeloLandUsers.FieldByName('UserGameScore').AsInteger < 2000) then
         Reg.VeloLandUsers.FieldByName('UserGameScore').AsInteger := 2000;
    Reg.VeloLandUsers.Post;
    SetLevel := '';
    MoveCounter.Caption := 'Количество ходов';
    ShowMessage('Вы успешно прошли уровень! Поздравляем!');
    Moves := 0;
  end;
end;

procedure TPicturesGame.TimerTimer(Sender: TObject); // таймер для ограничения по времени
var H,M,S:integer;
begin
i := i - 1;
H := I div 3600;
...
procedure TPicturesGame.TitleColorTimer(Sender: TObject); // переливание цветов текста "Игра "Парные картинки с велосипедами"
begin
