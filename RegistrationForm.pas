unit RegistrationForm;//модуль регистрации
...
procedure TReg.AuthButtonClick(Sender: TObject); //авторизация
begin
VeloLandUsers.Active := True;
if VeloLandUsers.Locate('UserLogin', Login.Text, []) then //проверка существования логина
  if VeloLandUsers.FieldByName('UserPassword').AsString = Pass.Text then //проверка пароля к логину
    begin
      MainForm.Show;
    end
  else
...
procedure TReg.AuthImgClick(Sender: TObject); //отображение полей авторизации
begin
...
procedure TReg.FormCreate(Sender: TObject); //загрузка картинок, подключение БД, расположение элементов упралвения
begin
...
SurnameLabel.Caption := 'Фамилия:';
SurnameLabel.Font.Name := 'Minecraft Rus Regular';
SurnameLabel.Font.Size := 16;
PassLabel.Caption := 'Пароль:';
PassLabel.Font.Name := 'Minecraft Rus Regular';
PassLabel.Font.Size := 16;
ConfrimPassLabel.Caption := 'Подтвердите пароль:';
ConfrimPassLabel.Font.Name := 'Minecraft Rus Regular';
ConfrimPassLabel.Font.Size := 12;
Title.Caption := 'Приветстувем Вас в приложении "Планета велосипедов"!';
Title.Font.Name := 'Minecraft Rus Regular';
Title.Font.Size := 20;
Title2.Caption := 'Для начала авторизируйтесь!';
Title2.Font.Name := 'Minecraft Rus Regular';
Title2.Font.Size := 20;
end;

procedure TReg.FormDestroy(Sender: TObject); //удаление шрифта из системы пользователя при закрытии
begin
   RemoveFontResource('resources\fonts\minecraft.ttf') ;
   SendMessage(HWND_BROADCAST, WM_FONTCHANGE, 0, 0) ;
end;

procedure TReg.N2Click(Sender: TObject); //вызов справки
begin
ShellExecute(0, PChar('Open'),PChar('VeloLandHelp.chm'),nil,nil,SW_SHOW);
end;

procedure TReg.N3Click(Sender: TObject); //окно "О программе"
begin
AProgramm.ShowModal;
end;

procedure TReg.RegButtonClick(Sender: TObject); //регистрация пользователя
begin
VeloLandUsers.Active := True;
if (Login.Text = '') or (Name.Text = '') or (Surname.Text = '') or (Pass.Text = '') or (ConfrimPass.Text = '') then //проверка на корректность ввода
  ShowMessage('Каждое поле должно быть заполнено!')
else if (Length(Pass.Text) < 4) then
  ShowMessage('Длина пароля не может быть меньше 4 символов!')
else if VeloLandUsers.Locate('UserLogin', Login.Text, []) then
  ShowMessage('Данный пользователь уже присутствует в системе! Перейдите на страницу авторизации!')
else
  begin //добавление нового пользователя в БД
    ...
    VeloLandUsers.Post;
    ShowMessage('Вы успешно зарегистрировались! Перейдите на страницу авторизации для входа в программу!');
  end;
Pass.Text := '';
ConfrimPass.Text := '';
end;

procedure TReg.RegImgClick(Sender: TObject); //отображение полей регистрации
begin
